...
function Signup() {
    ...
    <div className='flex justify-center items-center h-3/4'>
        <div className='p-5 w-1/3 rounded-2xl flex flex-col justify-center'>
            <h1 className='flex text-2xl font-semibold justify-center items-center'>Signup</h1>
            ...
            <div class="relative w-full mt-6">
                <input type={isvisible ? 'text' : 'password'} class="w-full bg-gray-800 text-gray-100 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400 py-2.5 px-3 rounded-md" placeholder="Confirm Password"/>
                <button onClick={togglePasswordVisibilty} class="absolute inset-y-0 right-0 flex items-center px-4 hover:text-white text-sm text-blue-300 focus:outline-none" type="button">{isvisible ? "Hide": "Show"}</button>
            </div>

            <button className='w-full mx-auto items-center justify-center outline-none mt-7 text-black bg-blue-300 px-8 py-2 text-sm rounded-3xl  hover:bg-blue-200 font-semibold'
            >Sign up</button>
            ...
            <button onClick={loginClick} className='text-blue-400 mt-5 hover:text-white text-xs'>Already a member? Login</button>
        </div>
    </div>
    </>
  )
